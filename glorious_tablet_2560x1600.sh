...
adb shell input swipe 1298 152 1730 202 126sleep 2

if [ \( $N -eq 0 \) -o \( $N -eq $(($NT+1)) \) ]; then
 echo "depositing ore, selecting bar $BAR"
 #deposit ore over action button #6
 adb shell input swipe 373 1465 497 1953 140 sleep 1
 #immaculate alloy
 adb shell input swipe 464 849 618 1132 144 sleep 1.2
 #glorious bar
 adb shell input swipe 1258 289 1677 385 158 sleep 1
fi
  #begin button over a blank right side #6 action bar button
adb shell input swipe 1952 1464 2602 1952 250
