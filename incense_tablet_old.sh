...
declare -i unit_input=27
if [ -n "$1" ]; then limit=$(($1/$unit_input+1)); fi
while [ \( $(($limit-$n)) -gt 0 \) -o \( $limit -eq 0 \) ]; do
  adb shell input swipe 762 584 762 584 1096
  sleep 1
  adb shell input swipe 1226 573 1226 573 175
  sleep 1
  adb shell input swipe 1460 384 1460 384 158
  sleep 1
  adb shell input swipe 1603 1061 1603 1061 197
  sleep 94
  n+=1
