...
while [ \( $(($limit-$n)) -gt 0 \) -o \( $limit -eq 0 \) ]; do
  ...
  outsph=$(bc <<< "scale=3; ($unit_input*1.10)/$spl * 3600")
  output="$spl s per loop, $n loops, $(($n * $unit_input)) ingredients, $outsph pots/hr"
  if [ $once -gt 0 ]; then
    now=$(date "+%r %a")
    echo "begin $now"
    once=0
  ...
  if [ $limit -gt 0 ]; then
    ...
    output+=" , eta $eta ($rh hours $rmi mins) $material"
  fi
  echo $output
done
